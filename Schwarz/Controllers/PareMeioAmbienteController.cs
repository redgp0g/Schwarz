    ...
    public class PareMeioAmbienteController : Controller
    {
        ...
        public async Task<IActionResult> Create(PareMeioAmbiente pareMeioAmbiente)
        {
            if (ModelState.IsValid)
            {
                ...
                string emailMessage = $"Uma nova falha foi cadastrada no site do PARE por {pareMeioAmbiente.Funcionario.Nome} cuja descrição é: {pareMeioAmbiente.Desvio} <br/>" + "Link do site:  <a href =\"http://192.168.2.96:5242/Pare/IndexQualidade\">Sistema Integrado</a>";
